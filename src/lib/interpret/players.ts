...
> = {
  GK: {
    roleKo: "골키퍼",
    styleKo:
      "골문을 지키고 수비 라인을 조율하며, 발밑 처리로 빌드업의 시작점이 되는 포지션입니다.",
    whyCareKo:
      "한 번의 선방이 승점을 바꾸는 자리 — 안정감이 곧 팀의 자신감입니다.",
  },
  DF: {
    roleKo: "수비수",
    styleKo:
      "상대 공격을 차단하고 위치 선정으로 위험을 줄이며, 후방에서 빌드업을 시작합니다.",
    whyCareKo: "탄탄한 수비 조직은 강팀이 꾸준할 수 있는 토대입니다.",
  },
  MF: {
    roleKo: "미드필더",
    styleKo:
      "공격과 수비를 연결하고 경기 템포를 조절하며, 압박과 전환의 중심이 됩니다.",
    whyCareKo: "중원을 잡는 팀이 경기의 흐름과 주도권을 가져갑니다.",
  },
  FW: {
    roleKo: "공격수",
    styleKo:
      "최전방에서 득점과 찬스 메이킹을 책임지며, 상대 수비를 끌어내 공간을 만듭니다.",
    whyCareKo: "결국 경기를 끝내는 건 골 — 공격진의 결정력이 승부를 가릅니다.",
  },
};
...
export function playerInsight(player: Player): PlayerInsight {
  ...
  // 3) Position template (always provides something).
  return { ...POSITION_FALLBACK[player.positionGroup], source: "seed" };
}

export function hasEditorialInsight(player: Player): boolean {
  return Boolean(getDossier(player.name)) || player.id in EDITORIAL;
}
